...
public abstract class ColorTuple implements IColorTuple {
	...
	public double compare(IColorTuple other) {
		...
		return Math.sqrt(
				Math.pow(myData[0] - otherData[0], 2)
				+ Math.pow(myData[1] - otherData[1], 2)
				+ Math.pow(myData[2] - otherData[2], 2));
	}
	...
	public String toString() {
		...
		return String.format("%02x", dToI(tuple[0]))
				+ String.format("%02x", dToI(tuple[1]))
				+ String.format("%02x", dToI(tuple[2]));
	}
